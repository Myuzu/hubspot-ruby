module Hubspot
  ...
  class Company
    ...
    class << self
      ...
      # @param params [Hash] Possible options are:
      ...
      def all(params = {}, opts = {})
        recently_updated = params.delete(:recently_updated) { false }
      ...
      # @return [Array] Array of Hubspot::Company records
      def find_by_domain(domain, params = {}, opts = {})
        raise Hubspot::InvalidParams, 'expecting String parameter' unless domain.try(:is_a?, String)

        limit = params.fetch(:limit, 100)
        properties = params.fetch(:properties) { Hubspot::CompanyProperties.all(opts).map { |property| property['name'] } }
        offset_company_id = params.fetch(:offset_company_id, nil)
      ...
      def create!(name, params = {}, opts = {})
        ...
        response = Hubspot::Connection.post_json(CREATE_COMPANY_PATH, { params: {}, body: post_data }, opts)
        new(response)
      end
    end
    ...
    def update!(params, opts = {})
      ...
      Hubspot::Connection.put_json(UPDATE_COMPANY_PATH, { params: { company_id: vid }, body: query }, opts)
    ...
    def destroy!(opts = {})
      Hubspot::Connection.delete_json(DESTROY_COMPANY_PATH, { company_id: vid }, opts)
      @destroyed = true
    end
    ...
    def contacts(opts = {})
      response = Hubspot::Connection.get_json(GET_COMPANY_CONTACTS_PATH, { company_id: vid }, opts)
      response['contacts'].each_with_object([]) do |contact, memo|
        memo << Hubspot::Contact.find_by_id(contact['vid'], opts)
      end
    end
  end
end
